...
object CellClusters {
  ...
  def main(args: Array[String]) {
    ...
    val inputFile = env.readCsvFile[Tower](if (params.has("input")) params.get("input") else "", fieldDelimiter = ",", includedFields = Array(0,1,4,6,7), ignoreFirstLine = true)

    var LTE_towers = inputFile
      .filter(_.radio.contains("LTE"))

    var other_towers = inputFile
      .filter(!_.radio.contains("LTE"))

    if(params.has("mnc")) {
      other_towers.filter(x => Array(params.get("mnc")).contains(x))
    }

    if(params.has("k")) {
      LTE_towers = LTE_towers
        .first(params.getInt("k"))
    }

    val centroids = LTE_towers.map(x => Centroid(x.cell, x.lat, x.lon))
    val points = other_towers.map(x => Point(x.lat, x.lon))
    ...
    if (params.has("output")) {
      clusteredPoints.map(x => (x._1, x._2.y, x._2.x)).writeAsCsv(params.get("output"), "\n", ",", writeMode = FileSystem.WriteMode.OVERWRITE).setParallelism(1)
  ...
  case class Tower(radio: String, mcc: Int, cell: Int, lon: Double,lat: Double)
